...
module.exports = {
    ...
    async run(bot, msg) {
        ...
        if (!reply || !reply.from) {
            return bot.sendMessage(msg.chat.id, "Please reply to the user's message to block them.");
        }
        ...
        if (db.blocked[userId]) {
            return bot.sendMessage(msg.chat.id, "User is already blocked.");
        }

        db.blocked[userId] = Date.now();
        saveDB();

        bot.sendMessage(msg.chat.id, `User ${reply.from.username || userId} has been blocked for 24 hours.`);

        try {
            await bot.sendMessage(reply.from.id, "You have been blocked from using this bot for 24 hours by an admin.");
        } catch (err) {
